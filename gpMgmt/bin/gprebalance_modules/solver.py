...
class GreedySolver:
    ...
    def solve(self) -> Tuple[Dict[int, Tuple[int, int]], int]:
        ...
        if self.run_improve:
            try:
                iters = 1000 #max(50, min(1000, self.n_segments // 5))
                time_limit = 60
    ...
    def _assign_mirrors(self, primary: List[int]) -> List[int]:
        ...
        if self.strategy == 'grouped':
            ...
            for p_host, segments in sorted_groups:
                ...
                # Find best mirror using clear criteria
                best_mirror = None
                ...
                # Priority 3: Fallback to least loaded (ignore capacity - shouldn't happen)
                if best_mirror is None:
                    best_mirror = min(
                        (h for h in range(self.n_hosts_target) if h != p_host),
                        key=lambda h: mirror_load[h]
                    )
    ...
    def _repair_with_regret(self, primary: List[int], mirror: List[int],
        ...
        while remaining:
            ...
            if self.strategy == 'spread':
                ...
                # (In practice, grouped usually has one mirror per primary group)
    ...
    def _repair_optimal(self, primary: List[int], mirror: List[int],
                   destroyed: Set[int]) -> Tuple[List[int], List[int]]:
        """
        Optimally repair destroyed segments.
        OPTIMIZED: Cache valid mirrors and iterate only over hosts with capacity.
        """
        new_primary = primary[:]
        new_mirror = mirror[:]
        ...
        # ===== OPTIMIZATION 1: Pre-build capacity lists =====
        primaries_with_capacity = set(h for h in range(self.n_hosts_target) 
                                      if primary_capacity[h] > 0)
        mirrors_with_capacity = set(h for h in range(self.n_hosts_target) 
                                    if mirror_capacity[h] > 0)

        # ===== OPTIMIZATION 2: Cache valid mirrors per primary host =====
        valid_mirror_cache = {}
        ...
        for seg in destroyed_list:
            ...
            # ===== EARLY EXIT: Try original placement first =====
            ...
            # ===== SEARCH: If not cost 0, try other primaries =====
            if best_cost > 0:
                # ===== USE CACHED CAPACITY SET =====
                for p_host in primaries_with_capacity:
                    if primary_capacity[p_host] <= 0:
...
if __name__=='__main__':
    ...
    #solver = GreedySolverOLD(20, 5, 5, conf[0], conf[1],
    #                   strategy='grouped')
    #solution, cost = solver.solve()
    
    conf = ConfigGenerator.generate_unbalanced_grouped(1000, 50)
    solver = GreedySolver(1000, 40, 50, conf[0], conf[1],
                       strategy='grouped', run_improve=True, printing=True)
    #solution, cost = solver.solve()
