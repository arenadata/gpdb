...
-- Test proper handling of volatile functions applied to Replicated locus,
-- when the plan with such locus is under the correlated/uncorrelated SubPlan.
-- Currently it's forbidden and the error should be thrown.
-- Volatile quals applied to the result of CTE. Correlated SubPlan.
explain (costs off, verbose)
with cte as (
    insert into with_dml_dr
    select i, i * 100 from generate_series(1,5) i
...
where t1.i in (select i from cte where cte.i = t1.j and cte.j > random());

-- Volatile quals applied to the result of CTE. Uncorrelated SubPlan.
explain (costs off, verbose)
with cte as (
    insert into with_dml_dr
    select i, i * 100 from generate_series(1,5) i
...
where t1.i in (select i from cte where cte.j > random());

-- Volatile returning list from modifying CTE.
explain (costs off, verbose)
with cte as (
    insert into with_dml_dr
    select i, i * 100 from generate_series(1,5) i
    returning i, j * random() as k
) select * from t1
where t1.i in (select k from cte where cte.i = t1.j);

-- Volatile target list from subplan with Replicated locus.
explain (costs off, verbose)
with cte as (
    insert into with_dml_dr
    select i, i * 100 from generate_series(1,5) i
...
where t1.i in (select random() from cte join t2 using (i) where t2.i = t1.j);

-- Volatile join quals, where join has locus Replicated.
explain (costs off, verbose)
with cte as (
    insert into with_dml_dr
    select i, i * 100 from generate_series(1,5) i
...
(select t2.i from cte join t2 on cte.i * random() = t2.i where t2.i = t1.j);

-- Test case when SubPlan contains node with Replicated locus, which is
-- propagated at different number of segments from outer relation. In this case
-- the Replicated node can't be properly broadcasted and the error will be thrown.
drop table if exists with_dml_dr_seg2;
--end_ignore
select gp_debug_set_create_table_default_numsegments(2);
create table with_dml_dr_seg2 (i int, j int) distributed replicated;
