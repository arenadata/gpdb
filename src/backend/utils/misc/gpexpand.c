...
int
GpRebalanceNumsegmentsShmemSize(void)
{
	return sizeof(*gp_create_table_rebalance_numsegments);
}

void
GpRebalanceNumsegmentsShmemInit(void)
{
	if (IsUnderPostmaster)
		return;

	/* only postmaster initialize it */
	gp_create_table_rebalance_numsegments = (volatile pg_atomic_uint32 *)ShmemAlloc(GpRebalanceNumsegmentsShmemSize());
