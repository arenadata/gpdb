...
/*
 * find_plangen_switch: find if plangen switched between statements.
 *
 * Find if plangen switched for statements of one plan or between plans.
 * 'gen' should point to variable, where very first statement's plangen value
 * will be stored and with what value all next plangens will be compared.
 * 'first_plan' value helps to indicate when we need to store initial plangen
 * value and when we need to compare it with existing one.
 */
static bool
find_plangen_switch(CachedPlan *plan, PlanGenerator *gen, bool first_plan)
{
	ListCell   *lc;
	foreach(lc, plan->stmt_list)
	{
		...
		if (first_plan)
		{
			*gen = plannedstmt->planGen;
			first_plan = false;
		}
		else if (*gen != plannedstmt->planGen)
			return true;
	}
	return false;
}
				...
				ParamListInfo boundParams, IntoClause *intoClause)
{
	...
	if (!plansource->plangen_switched)
		plansource->plangen_switched = find_plangen_switch(plan,
												&plansource->init_plangen_used,
												(plansource->generation == 0));
