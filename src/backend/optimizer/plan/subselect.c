...
typedef struct check_dml_context
{
	...
	bool		ignoreMaterialized;	 /* examine materialized ctes */
}			check_dml_context;
 ...
 */
bool
cte_contains_dml(Node *ctequery, PlannerInfo *root, bool ignoreMaterialized)
{
	 ...
	 * ignoreMaterialized indicates whether we need to traverse materialized
	 * CTEs.
	 */
	ctx.cteroot = root;
	ctx.m_query_level = root->query_level;
	ctx.ignoreMaterialized = ignoreMaterialized;
