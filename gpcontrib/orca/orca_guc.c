...
bool optimizer_xforms[OPTIMIZER_XFORMS_COUNT] = {
	[0 ... OPTIMIZER_XFORMS_COUNT - 1] = false};
...
struct config_bool configure_names_bool_orca[] = {
	{{"optimizer_log", PGC_USERSET, LOGGING_WHAT,
	  gettext_noop("Log optimizer messages."), NULL,
	...
	{{"optimizer_print_query", PGC_USERSET, LOGGING_WHAT,
	  gettext_noop("Prints the optimizer's input query expression tree."), NULL,
	  GUC_NO_SHOW_ALL | GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_print_plan", PGC_USERSET, LOGGING_WHAT,
	  ...
	  NULL, GUC_NO_SHOW_ALL | GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_print_xform", PGC_USERSET, LOGGING_WHAT,
	  gettext_noop("Prints optimizer transformation information."), NULL,
	  GUC_NO_SHOW_ALL | GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_print_missing_stats", PGC_USERSET, LOGGING_WHAT,
	  gettext_noop("Print columns with missing statistics."), NULL,
	  GUC_NO_SHOW_ALL | GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_print_memo_after_optimization", PGC_USERSET, LOGGING_WHAT,
	  gettext_noop("Print optimizer memo structure after optimization."), NULL,
	  GUC_NO_SHOW_ALL | GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_print_expression_properties", PGC_USERSET, LOGGING_WHAT,
	  gettext_noop("Print expression properties."), NULL,
	  GUC_NO_SHOW_ALL | GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_print_group_properties", PGC_USERSET, LOGGING_WHAT,
	  gettext_noop("Print group properties."), NULL,
	  GUC_NO_SHOW_ALL | GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_print_optimization_context", PGC_USERSET, LOGGING_WHAT,
	  gettext_noop("Print the optimization context."), NULL,
	  GUC_NO_SHOW_ALL | GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_print_optimization_stats", PGC_USERSET, LOGGING_WHAT,
	  gettext_noop("Print optimization stats."), NULL,
	  GUC_NO_SHOW_ALL | GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_extract_dxl_stats", PGC_USERSET, LOGGING_WHAT,
	  gettext_noop("Extract plan stats in dxl."), NULL,
	  GUC_NO_SHOW_ALL | GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_extract_dxl_stats_all_nodes", PGC_USERSET, LOGGING_WHAT,
	  gettext_noop("Extract plan stats for all physical dxl nodes."), NULL,
	  GUC_NO_SHOW_ALL | GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_enable_nljoin", PGC_USERSET, QUERY_TUNING_METHOD,
	  gettext_noop("Enable nested loops join plans in the optimizer."), NULL,
	  GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_enable_sort", PGC_USERSET, QUERY_TUNING_METHOD,
	  gettext_noop("Enable plans with Sort operators in the optimizer."), NULL,
	  GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_enable_partition_propagation", PGC_USERSET,
	  QUERY_TUNING_METHOD,
	  gettext_noop(
	...
	{{"optimizer_enable_direct_dispatch", PGC_USERSET, QUERY_TUNING_METHOD,
	  gettext_noop("Enable direct dispatch in the optimizer."), NULL,
	  GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_enable_space_pruning", PGC_USERSET, DEVELOPER_OPTIONS,
	  gettext_noop("Enable space pruning in the optimizer."), NULL,
	  GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_enable_hashjoin", PGC_USERSET, QUERY_TUNING_METHOD,
	  gettext_noop("Enables the optimizer's use of hash join plans."), NULL,
	  GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_enable_hashagg", PGC_USERSET, QUERY_TUNING_METHOD,
	  gettext_noop("Enables GPORCA to use hash aggregates."), NULL,
	  GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_enable_groupagg", PGC_USERSET, QUERY_TUNING_METHOD,
	  gettext_noop("Enables GPORCA to use group aggregates."), NULL,
	  GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_enable_derive_stats_all_groups", PGC_USERSET,
	  DEVELOPER_OPTIONS,
	...
	{{"optimizer_enable_multiple_distinct_aggs", PGC_USERSET,
	  QUERY_TUNING_METHOD,
	  gettext_noop(
	...
	{{"optimizer_enable_hashjoin_redistribute_broadcast_children", PGC_USERSET,
	  QUERY_TUNING_METHOD,
	  gettext_noop(
	...
	{{"optimizer_enable_broadcast_nestloop_outer_child", PGC_USERSET,
	  QUERY_TUNING_METHOD,
	  gettext_noop(
	...
	{{"optimizer_discard_redistribute_hashjoin", PGC_USERSET,
	  QUERY_TUNING_METHOD,
	  gettext_noop(
	...
	{{"optimizer_enable_mergejoin", PGC_USERSET, QUERY_TUNING_METHOD,
	  gettext_noop("Enables the optimizer's support of merge joins."), NULL,
	  GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_enable_gather_on_segment_for_dml", PGC_USERSET,
	  QUERY_TUNING_METHOD,
	  gettext_noop(
	...
	{{"optimizer_enumerate_plans", PGC_USERSET, LOGGING_WHAT,
	  gettext_noop("Enable plan enumeration"), NULL,
	  GUC_NO_SHOW_ALL | GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_sample_plans", PGC_USERSET, DEVELOPER_OPTIONS,
	  gettext_noop("Enable plan sampling"), NULL,
	  GUC_NO_SHOW_ALL | GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_enable_constant_expression_evaluation", PGC_USERSET,
	  QUERY_TUNING_METHOD,
	...
	{{"optimizer_enable_bitmapscan", PGC_USERSET, QUERY_TUNING_METHOD,
	  gettext_noop("Enable bitmap plans in the optimizer"), NULL,
	  GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_enable_outerjoin_to_unionall_rewrite", PGC_USERSET,
	  QUERY_TUNING_METHOD,
	  gettext_noop("Enable rewriting Left Outer Join to UnionAll"), NULL,
	  GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_enable_ctas", PGC_USERSET, QUERY_TUNING_METHOD,
	  gettext_noop("Enable CTAS plans in the optimizer"), NULL,
	  GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_enable_dml", PGC_USERSET, QUERY_TUNING_METHOD,
	  gettext_noop("Enable DML plans in GPORCA."), NULL, GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_use_gpdb_allocators", PGC_POSTMASTER, RESOURCES_MEM,
	  gettext_noop("Enable ORCA to use GPDB Memory Contexts"), NULL,
	  GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_enable_orderedagg", PGC_USERSET, DEVELOPER_OPTIONS,
	  gettext_noop("Enable ordered aggregate plans."), NULL},
	...
	{{"optimizer_enable_redistribute_nestloop_loj_inner_child", PGC_USERSET,
	  DEVELOPER_OPTIONS,
	  gettext_noop(
	...
	{{"optimizer_force_comprehensive_join_implementation", PGC_USERSET,
	  QUERY_TUNING_METHOD,
	  gettext_noop(
	...
	{{"optimizer_enable_replicated_table", PGC_USERSET, DEVELOPER_OPTIONS,
	  gettext_noop("Enable replicated tables."), NULL, GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_enable_foreign_table", PGC_USERSET, DEVELOPER_OPTIONS,
	  gettext_noop("Enable foreign tables in Orca."), NULL, GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_enable_motions_coordinatoronly_queries", PGC_USERSET,
	  QUERY_TUNING_METHOD,
	  gettext_noop(
		  "Enable plans with Motion operators in the optimizer for queries with no distributed tables."),
	  NULL, GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_force_three_stage_scalar_dqa", PGC_USERSET,
	  QUERY_TUNING_METHOD,
	  gettext_noop(
	...
	{{"optimizer_push_requirements_from_consumer_to_producer", PGC_USERSET,
	  DEVELOPER_OPTIONS,
	  gettext_noop(
	...
	{{"optimizer_enforce_subplans", PGC_USERSET, DEVELOPER_OPTIONS,
	  gettext_noop("Enforce correlated execution in the optimizer"), NULL,
	  GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_cte_inlining", PGC_USERSET, DEVELOPER_OPTIONS,
	  gettext_noop("Enable CTE inlining"), NULL, GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_apply_left_outer_to_union_all_disregarding_stats", PGC_USERSET,
	  DEVELOPER_OPTIONS,
	  gettext_noop(
	...
	{{"optimizer_remove_order_below_dml", PGC_USERSET, DEVELOPER_OPTIONS,
	  gettext_noop("Remove OrderBy below a DML operation"), NULL,
	  GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_enable_associativity", PGC_USERSET, QUERY_TUNING_METHOD,
	  gettext_noop("Enables Join Associativity in optimizer"), NULL},
	...
	{{"optimizer_enable_push_join_below_union_all", PGC_USERSET,
	  DEVELOPER_OPTIONS,
	  gettext_noop(
		  "Enable transform of join of union all to union all of joins. May improve the join performance."),
	  NULL, GUC_NOT_IN_SAMPLE},
...
struct config_int configure_names_int_orca[] = {
	{{"optimizer_plan_id", PGC_USERSET, DEVELOPER_OPTIONS,
	  gettext_noop("Choose a plan alternative"), NULL,
	  GUC_NO_SHOW_ALL | GUC_NOT_IN_SAMPLE},
	 &optimizer_plan_id,
	 0,
	 0,
	...
	{{"optimizer_samples_number", PGC_USERSET, DEVELOPER_OPTIONS,
	  gettext_noop("Set the number of plan samples"), NULL,
	  GUC_NO_SHOW_ALL | GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_penalize_broadcast_threshold", PGC_USERSET,
	  QUERY_TUNING_METHOD,
	  gettext_noop(
	...
	{{"optimizer_mdcache_size", PGC_USERSET, RESOURCES_MEM,
	  gettext_noop("Sets the size of MDCache."), NULL, GUC_UNIT_KB},
	...
	{{"optimizer_cte_inlining_bound", PGC_USERSET, QUERY_TUNING_METHOD,
	  gettext_noop("Set the CTE inlining cutoff"), NULL, GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_push_group_by_below_setop_threshold", PGC_USERSET,
	  QUERY_TUNING_METHOD,
	  gettext_noop(
	...
	{{"optimizer_xform_bind_threshold", PGC_USERSET, DEVELOPER_OPTIONS,
	  gettext_noop(
		  "Maximum number bindings per xform per group expression. A value of 0 disables."),
	  NULL, GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_skew_factor", PGC_USERSET, DEVELOPER_OPTIONS,
	  gettext_noop(
		  "Coefficient of skew ratio computed from sample stastics. Default 0: skew computation from sample statistics turned off. [1,100]: skew ratio computed from sample statistics. The skewness used for costing is the product of the optimizer_skew_factor and the skew ratio."),
	  NULL, GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_join_arity_for_associativity_commutativity", PGC_USERSET,
	  QUERY_TUNING_METHOD,
	  gettext_noop(
...
struct config_real configure_names_real_orca[] = {
	...
	{{"optimizer_nestloop_factor", PGC_USERSET, QUERY_TUNING_OTHER,
	  gettext_noop("Set the nestloop join cost factor in the optimizer"), NULL,
	  GUC_NOT_IN_SAMPLE},
	...
	{{"optimizer_jit_inline_above_cost", PGC_USERSET, QUERY_TUNING_COST,
	  ...
	  gettext_noop("-1 disables inlining."), GUC_EXPLAIN | GUC_GPDB_NEED_SYNC},
...
struct config_string configure_names_string_orca[] = {
	{{"optimizer_search_strategy_path", PGC_USERSET, QUERY_TUNING_METHOD,
	  gettext_noop("Sets the search strategy used by gp optimizer."), NULL,
	  GUC_NO_SHOW_ALL | GUC_NOT_IN_SAMPLE},
